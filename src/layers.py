...
class GraphConvLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True):
        ...
        if bias:
            self.bias = Parameter(torch.Tensor(out_features))  # Bias vector
    ...
    def forward(self, feature, adj):
        # Forward pass
        # feature: Node features of shape (batch_size, num_nodes, in_features)
        # adj: Adjacency matrix of shape (batch_size, num_nodes, num_nodes)
        support = torch.matmul(feature, self.weight)  # Apply linear transformation
        output = torch.matmul(adj, support)  # Aggregate neighbor information

        if self.bias is not None:
            return self.act(output + self.bias)  # Apply activation function
        else:
